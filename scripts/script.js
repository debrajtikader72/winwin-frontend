...
try {
const response = await fetch(`https://your-service-name.a.run.app/api/user-data/${currentUserId}`);
if (response.ok) {
...
if (!currentUserId) return;
try {
await fetch('http://localhost:5000/api/sync-wallet', {
method: 'POST',
...
function colorClassForDigit(d){if(d===0)return'c-red-violet';if(d===5)return'c-green-violet';if(d%2===0)return'c-red';return'c-green';}
...
let third = 0; 
try {
const res = await fetch(`https://your-service-name.a.run.app/api/period-result/${finishedPeriod}`);
if (res.ok) {
